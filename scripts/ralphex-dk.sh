...
def build_command(args: argparse.Namespace, ralphex_args: list[str]) -> list[str]:
    ...
    # Keep the container user aligned with the host user.
    cmd.extend([
        "-e", f"APP_UID={os.getuid()}",
        "-e", f"SKIP_HOME_CHOWN=1",
        "-e", "INIT_QUIET=1",
    ...
    # Claude credentials are optional. If ~/.claude does not exist, do not fail.
    claude_home = Path(os.environ.get("CLAUDE_CONFIG_DIR", str(home / ".claude"))).expanduser()
    add_mount(cmd, claude_home, "/home/app/.claude")

    # OpenCode credentials/config are also optional.
    opencode_data = home / ".local" / "share" / "opencode"
    opencode_config = home / ".config" / "opencode"
    add_mount(cmd, opencode_data, "/home/app/.local/share/opencode")
    add_mount(cmd, opencode_config, "/home/app/.config/opencode")

    # Ralphex config is always mounted if available; create it otherwise.
    ...
    # Codex credentials remain optional.
    codex_home = home / ".codex"
    add_mount(cmd, codex_home, "/home/app/.codex")
    ...
    if args.opencode:
        # --claude-command is a native ralphex override. The adapter ignores
        # Claude-specific flags it does not need, so no extra config is required.
...
def main() -> int:
    p = parser()
    args, ralphex_args = p.parse_known_args()
