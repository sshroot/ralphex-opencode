...
# Ralphex expects autonomous execution. Preserve user config while allowing all tools.
...
# Keep instructions in a temporary file so they do not replace the user's config.
tmp_dir=$(mktemp -d)
